...
def hcuco(i1, i2, prob):
    rs1 = []
    rs1.extend(random.choices(gSet, k=length))
    rss1 = list(rs1)
    rs2 = []
    rs2.extend(random.choices(gSet, k=length))
    rss2 = list(rs2)
    size = min(len(i1), len(i2))
    for i in range(size):
        if random.random() < prob:
          #  i1[i], i2[i] = i2[i], i1[i]
            i1[i], rss1[i] = rss1[i], i1[i]
            i2[i], rss2[i] = rss2[i], i2[i]
...
def hcspc(i1, i2):
    ...
    i1[cop:], rss1[cop:] = rss1[cop:], i1[cop:]
    i2[cop:], rss2[cop:] = rss2[cop:], i2[cop:]
...
def hcmpc(i1, i2):
    ...
    if cop1 != cop2:
            #cop1, cop2 = cop2, cop1
        i1[cop1:cop2], rss1[cop1:cop2] = rss1[cop1:cop2], i1[cop1:cop2]
        i2[cop1:cop2], rss2[cop1:cop2] = rss2[cop1:cop2], i2[cop1:cop2]
        ...
        ##        print(len(i2))
                ...
                if leni1 == len(proof): #i in (Fitness(i1), Fitness(proof)):
                   ...
                   bFitness = Fitness(i1)
                    ...
                    #print("PARENT 2:", i2)
                    sTime = time.time()
                    bParent, bFitness = hcuco(i1, i2, 0.5)
                ...
                while True:
                    ...
                    #     if len(totalIterations) % 5000 == 0:
                    #   print (len(totalIterations), cFitness)
